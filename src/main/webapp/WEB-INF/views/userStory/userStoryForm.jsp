...
<t:layout userName="${username}" userRole="${role}"
	contentTitle="${title}" activeMenuUserStories="active">


	<form:form class="form-horizontal" action="userStoryForm" method="post"
		commandName="userStoryBean">
		<div class="form-group">
			<label for="inputName" class="col-sm-2 control-label">User
				story name</label>
			<div class="col-sm-10">
				<form:input type="text" name="name" class="form-control" path="name"
					id="inputName" placeholder="User story name"
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<form:select path="productId" name="productId" id="productId"
					value="${userStory.product.id}" class="form-control">
					<c:forEach items="${productList}" var="productList">
						<c:choose>
							<c:when test="${userStory.product.id == productList.id}">
								<form:option value="${productList.id}"
									selected="${userStory.product.id}">${productList.name} </form:option>
							</c:when>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				...
				<form:select path="releaseId" name="releaseId" id="releaseList"
					class="form-control">
				</form:select>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				...
				<form:select path="sprintId" name="sprintId" id="sprintList"
					class="form-control">
				</form:select>
		...
		<div class="form-group">
			<label for="inputName" class="col-sm-2 control-label">Start
				date</label>
			<div class="col-sm-10">
				<form:input type="text" name="startDate"
					class="datePicker form-control" path="startDate"
					id="inputStartDate" placeholder="yyyy-mm-dd"
					value="${userStory.formatStartDate()}" />
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<form:input type="text" class="datePicker form-control"
					id="inputDueDate" placeholder="yyyy-mm-dd" name="dueDate"
					value="${userStory.formatDueDate()}" path="dueDate" />
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<form:select path="priority" name="priority" id="inputPriority"
					value="${userStory.priority}" class="form-control">
					<c:forEach items="${priority}" var="priority">
						<c:choose>
							<c:when test="${userStory.priority == priority}">
								<form:option value="${priority}"
									selected="${userStory.priority}">${priority} </form:option>
							</c:when>
		...
		<div class="form-group">
			<label for="inputName" class="col-sm-2 control-label">Estimate
				development effort</label>
			<div class="col-sm-10">
				<form:input type="text" class="form-control"
					id="inputEstimateDevEffort"
					placeholder="Estimate development effort" name="estimateDevEffort"
					value="${userStory.estimateDevEffort}" path="estimateDevEffort" />
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<form:input type="text" class="form-control"
					id="inputEstimateTestEffort" placeholder="Estimate test effort"
					name="estimateTestEffort" value="${userStory.estimateTestEffort}"
					path="estimateTestEffort" />
				<form:errors path="estimateTestEffort" cssClass="error"></form:errors>
			</div>
		</div>

		<div class="form-group">
			<label for="inputName" class="col-sm-2 control-label">Assinged
				developer</label>
			<div class="col-sm-10">
				<form:select path="developerId" name="developerId"
					id="inputDeveloperId" value="${userStory.developerId.id}"
					class="form-control">
					<c:forEach items="${developer}" var="developer">
						<form:option value="${developer.id}"
							selected="${userStory.developerId.id}">${developer.getFullname()}</form:option>
					</c:forEach>
				</form:select>
		...
		<div class="form-group">
			<label for="inputName" class="col-sm-2 control-label">Assigned
				tester</label>
			<div class="col-sm-10">
				<form:select path="testId" name="testId" id="inputTestId"
					value="${userStory.testId.id}" class="form-control">
					<c:forEach items="${tester}" var="tester">
						<form:option value="${tester.id}"
							selected="${userStory.testId.id}">${tester.getFullname()}</form:option>
					</c:forEach>
				</form:select>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<form:textarea name="description" row="5" class="form-control"
					id="inputDescription" placeholder="Description" path="description" />
			</div>
		</div>
	...
	<script>
		...
		function getReleaseList(productId) {
			$
					.getJSON(
							"getReleasesByProductId?productId=" + productId,
							function(jsonData) {
								var options = '';
								...
								for (var i = 0; i < releases.length; i++) {
									if (releases[i].id == defaultRelease)
										options += '<option value="' + releases[i].id + ' " selected>'
												+ releases[i].name
												+ '</option>';
									else
		...
		function getSprintList(releaseId) {
			$
					.getJSON(
							"getSprintsByReleaseId?releaseId=" + releaseId,
							function(jsonData) {
								var options = '';
								...
								for (var i = 0; i < sprints.length; i++) {
									if (sprints[i].id == defaultSprint)
										options += '<option value="' + sprints[i].id + ' " selected>'
												+ sprints[i].name + '</option>';
									else
										options += '<option value="' + sprints[i].id + '">'
												+ sprints[i].name + '</option>';
								}
		...
		$(function() {
			...
			$("#inputStartDate").change(function() {
				...
				$("#inputStartDate").val(startDate.substring(4, 14));

			});

			$("#inputDueDate").change(function() {
				var endDate = $("#inputDueDate").val();
				$("#inputDueDate").val(endDate.substring(4, 14));
